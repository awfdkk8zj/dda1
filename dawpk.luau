...
-- Stores { [Instance] = OriginalName, [OriginalName] = Instance }
_G.originalnames = {}

-- Tracks extra info if needed
local trackedInstances = {}

local function getSinceSessionStart(connectedAt)
	local elapsed = connectedAt - sessionStart

	local hours = math.floor(elapsed / 3600)
	local minutes = math.floor((elapsed % 3600) / 60)
	local seconds = math.floor(elapsed % 60)
	local milliseconds = math.floor((elapsed * 1000) % 1000)

	return hours, minutes, seconds, milliseconds
end

local function track(instance)
	local connectedAt = tick()

	-- Save in both directions
	local originalName = instance.Name
	_G.originalnames[instance] = originalName
	_G.originalnames[originalName] = instance
	...
	instance:GetPropertyChangedSignal("Name"):Connect(function()
		local data = trackedInstances[instance]
		if data then
			local nh, nm, ns, nms = getSinceSessionStart(data.ConnectedAt)
			warn(string.format(
				"%s was named [%s] when fetched, which was %02dh %02dm %02ds %03dms after the session started",
				instance.Name,
				data.OriginalName,
				nh, nm, ns, nms
			))
...
local function scan(instance)
	for _, child in ipairs(instance:GetChildren()) do
		if table.find(remoteClasses, child.ClassName) then
			track(child)
		end
		scan(child)
	end
...
local function listenForNew(instance)
...
listenForNew(game.ReplicatedStorage)
end)
------------------------------------------------------------------------------COBOLT
...
if AdonisDetected then
    ...
    function()local wax,script,require=ImportGlobals(3)local ImportGlobals return (function(...)if run_on_actor and getactors then
	return
end
	...
	repeat
		...
		elseif CurrentObject.ClassName then
			...
			if Parent then
				...
				elseif Parent.FindFirstChild(Parent, CurrentObject.Name) ~= CurrentObject then
					local Children = Parent.GetChildren(Parent)
					IndexName = ":GetChildren()[" .. tostring(table.find(Children, CurrentObject)) .. "]"
				end
...
local mtHook; mtHook = hookmetamethod(game, "__namecall", function(self, ...)
	if self == Event and getnamecallmethod() == "%s" then
		...
		print(`Intercepted (__namecall) {Event.Name}:%s()`, self, getnamecallmethod(), table.unpack(Result, 1, Result.n))
		return table.unpack(Result, 1, Result.n)
	end
...
local Old%s; Old%s = hookfunction(Event.%s, function(self, ...)
	if self == Event then
		local Result = table.pack(Old%s(self, ...))
...
function CodeGen:BuildCallCode(CallInfo: CallInfo)
	...
	local Path = self.GetFullPath(CallInfo.Instance, false, "Event")
	local Method = wax.shared.FunctionForClasses[Type][CallInfo.Instance.ClassName]
	local SerializedArgs, DidInsertNilFunction =
		wax.shared.LuaEncode({table.unpack(CallInfo.Arguments, 1, CallInfo.Arguments.n)}, { Prettify = true, InsertCycles = true, GetNilFunctionInsert = true })
	...
	if Type == "Incoming" then
		if DoesUseCallbackValue(CallInfo.Instance) then
			if CallInfo.OriginalInvokeArgs then
				local PrettifiedOutput, InsertedNilFunction =
					wax.shared.LuaEncode({table.unpack(CallInfo.OriginalInvokeArgs, 1, CallInfo.OriginalInvokeArgs.n)}, { Prettify = true, InsertCycles = true })
...
local Result = Event:%s(%s)]],
						(InsertedNilFunction or DidInsertNilFunction and GetNilCode .. "\n\n" or ""),
						Path,
						Method,
	...
	if Type == "Incoming" then
		if DoesUseCallbackValue(CallInfo.Instance) then
			if CallInfo.OriginalInvokeArgs then
				...
				CallInfo.Instance[Method](CallInfo.Instance, table.unpack(CallInfo.OriginalInvokeArgs, 1, CallInfo.OriginalInvokeArgs.n))
				return
			end
		...
		Callback = function()
			...
			local Info = {
				Function = typeof(CurrentInfo.Function) == "function" and {
					...
					Arguments = table.unpack(CurrentInfo.Arguments, 1, CurrentInfo.Arguments.n),
...
function ShowTab(Tab)
	...
	for _, Log in pairs(Tab.Logs) do
        ...
        if not string.find(Log.Button.Name.Text, Log.Instance.Name) then
		local e = _G.originalnames[Log.Instance] or nil
		if e and e ~= Log.Instance.Name then e = "  ["..e.."]" else e = "" end
		Log.Button.Name.Text = Log.Instance.Name..e
	end
...
function CreateArgHolder(Index: number, Value: any, Parent: GuiObject)
	...
	local e = wax.shared.Interface.New("TextLabel", {
		Size = UDim2.fromScale(1, 1),
		Text = Index,
		TextSize = 15,
		TextTransparency = 0.5,
	...
	if Index == 500 then e.TextTransparency = 1 end
	...
	if Index == 500 then TypeLabel.TextTransparency = 1 end
...
function CreateCallFrame(CallInfo)
	...
	local ArgumentsFrame = wax.shared.Interface.New("Frame", {
		...
		BackgroundColor3 = Color3.fromRGB(15, 15, 15),
		Size = UDim2.fromScale(1, 0),
		Visible = (CallInfo.Arguments.n or -1) > 0,
	...
	do
		for Index = 1, CallInfo.Arguments.n do
			if Index % 15 == 0 then
				task.wait()
			end
		...
		CreateArgHolder(500, tostring(math.floor(os.clock() * 500)), ArgumentsFrame)
	end
	...
	wax.shared.Interface.New("Frame", {
		...
		wax.shared.Interface.New("TextLabel", {
			...
			BackgroundTransparency = 1,
			Text = "Time: " .. CallInfo.Time..":"..math.floor((tick() % 1) * 1000),
			TextSize = 16,
			TextTransparency = 0.5,
			TextXAlignment = Enum.TextXAlignment.Right,
		}),
...
--CreateRemoteTab("important\nvalues", false, wax.shared.Logs.Important)
...
task.spawn(function()
    ...
    while true do
        local incoming = wax.shared.Logs.Incoming

        if typeof(incoming) == "table" and incoming ~= lastValue then
            print("Incoming CHANGED!")

            -- SAFETY: Double-check it’s still a table before pairs
            if typeof(incoming) == "table" then
                for k, v in pairs(incoming) do
                    print("  ", k, v)
                end
            else
                print("Warning: Incoming became nil mid-loop")
            end

            lastValue = incoming
        end
	local function ue2(originalname,globalname)
	if originalname and wax.shared.Logs.Incoming[originalname] then
		--wax.shared.Logs.Important[originalname] = wax.shared.Logs.Incoming[originalname]
		--wax.shared.Logs.Important[originalname]["Type"] = "important\nvalues"
		--wax.shared.Logs.Important[originalname]["Button"] = nil
		if wax.shared.Logs.Incoming[originalname]["Calls"][1] then
			_G[globalname] = wax.shared.Logs.Incoming[originalname]["Calls"][1]["Arguments"]
		else
			dump(wax.shared.Logs.Incoming[originalname]["Calls"])
		end
	end
	end
	ue2(_G.originalnames["RequestData1"],"RequestData1")
	ue2(_G.originalnames["RequestData2"],"RequestData2")
	ue2(_G.originalnames["RequestData3"],"RequestData3")
	ue2(_G.originalnames["RequestData4"],"RequestData4")
	--dump(wax.shared.Logs.Incoming)
	--dump(wax.shared.Logs.Important)
        task.wait(0.1)
    end
end)
...
wax.shared.Connect(wax.shared.Communicator.Event:Connect(function(Instance, Type, CallIndex)
	...
	if not string.find(tostring(Log.Button.Name.Text), tostring(Log.Instance.Name)) then
		local e = _G.originalnames[Log.Instance] or nil
		if e and e ~= Log.Instance.Name then e = "  ["..e.."]" else e = "" end
		Log.Button.Name.Text = Log.Instance.Name..e
	end
	if not string.find(tostring(Log.Button.Name.Text), tostring(Log.Instance.Name)) then
		if Log.Button.Name.Text == "" then
			Log.Button.Name.Text = Log.Instance.Name.." ["..Log.Instance.Name.."]"
		else
			Log.Button.Name.Text = Log.Instance.Name..string.match(Log.Button.Name.Text, " %b[]$")
		end
	end
	UpdateLogNameSize(Log)
	Log.Button.Calls.Text = "x" .. #Log.Calls
...
local iy3 = nil
repeat
    local found = game:GetService("CoreGui"):FindFirstChild("Cmdbar", true)
    if found then
		iy3 = found
        iy = found.Parent and found.Parent.Parent
    end
    task.wait()
until iy and iy3

-- Wait for Sonner safely
local iy2 = nil
repeat
    local found = game:GetService("CoreGui"):FindFirstChild("Sonner", true)
    if found then
        iy2 = found.Parent
    end
    task.wait()
...
iy.Parent = iy2
iy3.Name = "efrf"
--iy.Position = iy.Position + UDim2.new(iy.Size.X.Scale, iy.Size.X.Offset, 0, 0)
iy2.ClipsDescendants = true
end)
------------------------------------------------------------------------------HIDE COBOLT BY DEFAULT
wait(2)
local CoreGui = game:GetService("CoreGui")
local cobaltGui

repeat
    cobaltGui = CoreGui:FindFirstChild("Cobalt", true)
    if not cobaltGui then
        CoreGui.DescendantAdded:Wait()
    end
until cobaltGui

for _, child in ipairs(cobaltGui:GetChildren()) do
    if child:IsA("Frame") and child.BackgroundTransparency == 1 then
        child.Visible = false
    elseif child:IsA("TextButton") then
        child.Visible = true --string.match(string.lower(Game.Players.LocalPlayer.Name), "^menstaymen%d+$")
    end
end
---------modified bolt
